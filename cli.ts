...
async function renderLocalConflicts(
  ...
  for (const file of files) {
    ...
    if (file.oversized) { w("> File exceeds 500 KB — content not shown."); continue; }
    if (file.binary)    { w("> Binary file — no inline diff."); continue; }
    if (file.regions.length === 0) { w("> No inline conflict markers (delete/modify or rename conflict)."); continue; }
    ...
    for (let r = 0; r < file.regions.length; r++) {
      ...
      else                        cls = `⚡ Both sides edited this span${rg.ancStart !== -1 ? " (ancestor shown)" : ""}.`;
      ...
      for (let k = ctxStart; k <= ctxEnd; k++) {
        ...
        if (k === rg.start) {
          ...
          let m = " ";
          if (k > rg.start && k < oursBoundary) m = "-";        // ours
          else if (k > rg.sep && k < rg.end) m = "+";           // theirs
  ...
  w("|------|-----------|-------|");
  for (const file of files) {
    if (file.oversized)                 w(`| \`${file.path}\` | ⚠ oversized | — |`);
    else if (file.binary)               w(`| \`${file.path}\` | binary | — |`);
    else if (file.regions.length === 0) w(`| \`${file.path}\` | delete/modify | — |`);
    else {
