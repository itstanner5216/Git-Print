...
function regionRangeLabel(file, rg) {
    ...
    if (sm) {
        ...
        for (let n = rg.startLine; n <= rg.endLine; n++) {
            ...
            if (e?.ours != null)
                ours.push(e.ours);
            if (e?.theirs != null)
                theirs.push(e.theirs);
        }
        if (ours.length)
            return `ours L${Math.min(...ours)}–${Math.max(...ours)}`;
        if (theirs.length)
            return `theirs L${Math.min(...theirs)}–${Math.max(...theirs)}`;
    }
...
function workingTreeSideMap(dir, relPath, content) {
    try {
        let combined;
        try {
            const cc = execFileSync("git", ["diff", `-U${SIDEMAP_CONTEXT}`, "--", relPath], { cwd: dir, encoding: "utf-8", maxBuffer: 64 * 1024 * 1024,
...
function composeConflictMarkdown(files, opts) {
    ...
    for (const file of files) {
        ...
        for (let r = 0; r < file.regions.length; r++) {
            ...
            const oursAt = (n) => sideMap ? (sideMap.get(n)?.ours ?? null) : n;
            const theirsAt = (n) => sideMap ? (sideMap.get(n)?.theirs ?? null) : n;
            const baseAt = (n) => sideMap ? (sideMap.get(n)?.base ?? null) : null;
            ...
            for (const t of rg.ctxBefore) {
                gutterRows.push({ num: oursAt(ln), text: `   ${t}` });
                ln++;
            }
            gutterRows.push({ num: null, text: `  ${rule(`<<<<<<< OURS · ${oursLabel}`, "═")}` });
            ln++;
            ...
            if (rg.ancestorLines !== null) {
                gutterRows.push({ num: null, text: `  ${rule("||||||| BASE · common ancestor", "─")}` });
                ln++;
                for (const t of rg.ancestorLines) {
                    gutterRows.push({ num: baseAt(ln), text: `   ${t}` });
                    ln++;
                }
            }
            gutterRows.push({ num: null, text: `  ${rule(`======= THEIRS · ${theirsLabel}`, "═")}` });
            ln++;
            for (const t of rg.theirsLines) {
                gutterRows.push({ num: theirsAt(ln), text: ` + ${t}` });
                ln++;
            }
            gutterRows.push({ num: null, text: `  ${rule(">>>>>>> END", "═")}` });
            ln++;
            ...
            const g = (n) => (n != null ? String(n) : "").padStart(width);
            w("```diff");
            for (const row of gutterRows)
                out.push(`${g(row.num)}${row.text}`.replace(/\s+$/, ""));
            w("```");
            w();
        }
    }
...
`;
function repoIsGit(path) {
    try {
        execSync("git rev-parse --git-dir", { cwd: path, stdio: ["pipe", "pipe", "pipe"] });
        return true;
    }
    catch {
        return false;
...
function repoHasGitHubRemote(path) {
    try {
        const out = execSync("git remote -v", { cwd: path, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
 ...
 */
function writeCiWorkflows(dryRun = false) {
    ...
    for (const { alias, path } of repos) {
        ...
        if (!existsSync(path)) {
            console.log(`   ⫯ ${label} path missing — skipped`);
            skipped++;
            continue;
        }
        if (!repoIsGit(path)) {
            console.log(`   ⫯ ${label} not a git repo — skipped`);
            skipped++;
            continue;
        }
        if (!repoHasGitHubRemote(path)) {
            console.log(`   ⫯ ${label} no github.com remote — skipped`);
            skipped++;
            continue;
        }
        ...
        if (existed)
            ...
            wrote++;
        if (dryRun) {
            console.log(`   ${existed ? "↻" : "✓"} ${label} ${dest}  (dry-run)`);
            continue;
        }
    ...
    if (hasConfigHooks) {
        ...
        if (existsSync(hookPath)) {
            ...
            if (src.includes(marker)) {
                console.log(`   ✓ already in ${hookPath}`);
                return;
            }
...
function removeCiWorkflows() {
    ...
    for (const { alias, path } of getRepos()) {
        ...
        if (!existsSync(dest))
            continue;
        try {
            const src = readFileSync(dest, "utf-8");
...
function uninstallConflictHook() {
    const { major, minor } = getGitVersion();
    const hasConfigHooks = major > 2 || (major === 2 && minor >= 54);
    if (hasConfigHooks) {
        try {
            ...
            console.log(`   (no hook in ~/.gitconfig)`);
        }
    }
    else {
        const gitRoot = getGitRoot(process.cwd());
...
async function runCiStatus(args) {
    let prNumber = null;
    let sha = null;
