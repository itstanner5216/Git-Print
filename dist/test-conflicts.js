...
import { extractConflicts, validateInWorktree, resolveConflicts, } from "./pr-renderer.js";
// ─── Helpers ────────────────────────────────────────────────────────────────
...
// ─── Tests ──────────────────────────────────────────────────────────────────
function testExtractConflicts(repoDir) {
...
function testValidateAndResolve(repoDir) {
...
function testBadResolution(repoDir) {
...
function testTypoDetection(repoDir) {
...
function testFullResolve(repoDir) {
    ...
    if (result.status === "committed") {
        ...
        try {
            git(`merge-base --is-ancestor ${headBefore} HEAD`, repoDir);
            ancHead = true;
        }
        catch { /* not ancestor */ }
        try {
            git(`merge-base --is-ancestor base-branch HEAD`, repoDir);
            ancBase = true;
        }
        catch { /* not ancestor */ }
...
function testRefuseWrongBranch() {
...
function testPartialResolve() {
    ...
    try {
        git("rev-parse -q --verify MERGE_HEAD", tmpDir);
    ...
    // Clean up
    try {
        git("merge --abort", tmpDir);
    }
    catch { /* fine */ }
    rmSync(tmpDir, { recursive: true, force: true });
}
function testDeletionResolution() {
