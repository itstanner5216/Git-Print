...
export interface PRRendererOptions {
  ...
  outputPath: string;
  graphqlToken?: string;
  includeResolvedThreads?: boolean; // default: true
  fetchCheckAnnotations?: boolean;  // default: true
  gitRoot?: string; // local clone of the repo — enables verifying/deriving
                    // line numbers for bot diffs against the real blobs
}
...
export interface PRMetadata {
  ...
  base: { ref: string; sha: string; label: string; repo?: { full_name?: string; clone_url?: string } };
  head: { ref: string; sha: string; label: string; repo?: { full_name?: string; clone_url?: string; fork?: boolean } };
...
/**
 * One conversation card in scroll order. Reviews, issue comments and inline
 * review threads are all flattened into this single sequence and numbered
 * "Comment N" top-to-bottom — mirroring how GitHub stacks them.
 */
interface Card {
  kind: "issue_comment" | "review_event" | "inline_thread";
  timestamp: string;
  data?: any;            // issue_comment / review_event payload
  thread?: ThreadGroup;  // inline_thread payload
  threads?: ThreadGroup[]; // inline threads submitted WITH a review_event —
                           // the UI shows a review (summary + its inline
                           // comments) as ONE timeline card, never as
                           // separate comments.
}
...
export type SideLineMap =
  Map<number, { ours: number | null; theirs: number | null; base: number | null }>;
...
export interface ResolveOptions {
  ...
  head: string;   // head branch name (e.g. "feat/xyz")
...
export interface ResolveResult {
  ...
  resolved?: { file: string; side: string }[];
  skipped?: string[];
  warnings?: string[];
...
function updateRateLimit(headers: Headers, rl: RateLimitState): void {
  const rem = headers.get("x-ratelimit-remaining");
  const reset = headers.get("x-ratelimit-reset");
  if (rem !== null) rl.remaining = parseInt(rem, 10);
  if (reset !== null) rl.resetAt = parseInt(reset, 10);
}

async function waitForRateLimit(rl: RateLimitState): Promise<void> {
  if (rl.remaining < 10) {
...
export async function fetchAllPages<T = any>(
  ...
  while (url) {
    await waitForRateLimit(rl);
    const res: Response = await fetch(url, { headers: pageHeaders });
    updateRateLimit(res.headers, rl);
    if (!res.ok) {
...
async function graphqlFetch(
  query: string, variables: Record<string, any>, token: string,
  rl: RateLimitState = createRateLimitState()
): Promise<any> {
  await waitForRateLimit(rl);
...
export async function fetchPRMetadata(
  owner: string, repo: string, pullNumber: number, token: string
): Promise<PRMetadata> {
...
async function fetchResolvedThreads(
  owner: string, repo: string, pr: number, token: string, rl: RateLimitState
): Promise<ResolvedThreadMap> {
  const map: ResolvedThreadMap = {};
  const query = `
...
function mergeChecksAndStatuses(checkRuns: any[], statuses: any[]): UnifiedCheck[] {
  const byName = new Map<string, UnifiedCheck>();
...
export async function fetchAllPRData(
  ...
  let resolvedThreadMap: ResolvedThreadMap | null = null;
  if (graphqlToken) {
    fetchPromises.push(
      fetchResolvedThreads(owner, repo, pullNumber, graphqlToken, rl).catch((e: any) => {
  ...
  try {
    const pageUrl = (pr as any).html_url || `https://github.com/${owner}/${repo}/pull/${pullNumber}`;
...
function formatAssociation(assoc: string, user: any): string {
...
function labelAssociation(assoc: string, user: any): string {
 ...
 */
function extractRangeFromHunk(
  diffHunk: string, startLine: number, endLine: number
): string[] | null {
  ...
  if (headerIdx < 0 || newStart === null) return null;
...
function parseSuggestions(
  body: string, diffHunk: string, startLine: number | null, endLine: number | null
): Suggestion[] {
...
interface ParsedHunk {
  ...
  oldStart: number;
  newStart: number;
  heading: string;   // text trailing the closing "@@" on the header line
  body: string[];    // lines after the header
}

/** Parse the first @@ hunk header in a diff_hunk. */
function parseHunkHeader(diffHunk: string): ParsedHunk | null {
  const lines = diffHunk.split("\n");
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@(.*)$/);
    if (m) {
...
/** A single diff row resolved into old/new line-number space. */
interface NumberedRow {
  oldNum: number | null;
  newNum: number | null;
  marker: " " | "+" | "-";
  content: string; // source text with the leading diff marker stripped
}
 ...
 */
function numberHunkRows(hunk: ParsedHunk): NumberedRow[] {
  const rows: NumberedRow[] = [];
  let oldNum = hunk.oldStart;
  let newNum = hunk.newStart;
  for (const l of hunk.body) {
    if (l.startsWith("@@")) break; // a second hunk header — stop here
    if (l.startsWith("-")) {
...
function diffLines(
  a: string[], b: string[]
): { type: "context" | "del" | "add"; text: string }[] {
  ...
  const out: { type: "context" | "del" | "add"; text: string }[] = [];
  let i = 0, j = 0;
  while (i < n && j < m) {
 ...
 */
function renderNumberedRows(rows: NumberedRow[]): string[] {
...
function collectAutomatedSuggestions(node: any, into: Map<number, any[]>): void {
  if (!node || typeof node !== "object") return;
  if (Array.isArray(node)) {
    for (const item of node) collectAutomatedSuggestions(item, into);
    return;
  }
  ...
  for (const key of Object.keys(node)) collectAutomatedSuggestions(node[key], into);
}
...
function scanEmbeddedData(html: string, into: Map<number, any[]>): void {
...
async function fetchGithubHtml(url: string): Promise<string | null> {
  try {
 ...
 */
export async function attachAutomatedSuggestions(
  prHtmlUrl: string | undefined, reviewComments: any[]
): Promise<void> {
  if (!prHtmlUrl || reviewComments.length === 0) return;
  ...
  for (const frag of fragments) {
    if (frag !== null) scanEmbeddedData(frag, recovered);
  }

  if (recovered.size === 0) return;

  const byId = new Map<number, any>(reviewComments.map((c: any) => [c.id, c]));
  let attached = 0;
 ...
 */
function automatedSuggestionDiffBlocks(comment: any): { path: string | null; lines: string[] }[] {
  const out: { path: string | null; lines: string[] }[] = [];
  ...
  for (const entry of entries) {
    ...
    for (const l of entry?.diffLines || []) {
      ...
      const marker: " " | "+" | "-" =
        l?.type === "DELETION" ? "-" : l?.type === "ADDITION" ? "+" : " ";
      rows.push({
    ...
    if (rows.length > 0) out.push({ path: entry?.path ?? null, lines: renderNumberedRows(rows) });
  }
  return out;
}
 ...
 */
function codeContextDiffLines(diffHunk: string, rangeSize: number): string[] {
 ...
 */
function suggestionDiffBlocks(
  body: string, diffHunk: string,
...
function actorSuffix(user: any): string {
  if (!user) return "";
  if (/copilot/i.test(user.login || "")) return " [AI]";
  if (user.type === "Bot" || /\[bot\]$/i.test(user.login || "")) return " [Bot]";
  return "";
}
...
function displayActor(user: any): string {
...
function actorEmoji(user: any): string {
  if (!user) return "🧑";
  if (/copilot/i.test(user.login || "")) return "🦾";
  if (user.type === "Bot" || /\[bot\]$/i.test(user.login || "")) return "🤖";
  return "🧑";
}
...
function buildCards(
  issueComments: any[], reviews: any[], reviewComments: any[],
  resolvedMap: ResolvedThreadMap | null, includeResolved: boolean
): Card[] {
  const cards: Card[] = [];
  ...
  const threadRoots = new Map<number, any>();
  const threadReplies = new Map<number, any[]>();
  for (const rc of reviewComments) {
    if (rc.in_reply_to_id == null) {
      threadRoots.set(rc.id, rc);
      if (!threadReplies.has(rc.id)) threadReplies.set(rc.id, []);
    } else {
      const rootId = rc.in_reply_to_id;
      if (!threadReplies.has(rootId)) threadReplies.set(rootId, []);
  ...
  const threadsByReview = new Map<number, ThreadGroup[]>();
  const orphanThreads: ThreadGroup[] = [];
  for (const [rootId, root] of threadRoots) {
    ...
    if (resolvedMap !== null) isResolved = resolvedMap[root.id] ?? null;
    if (!includeResolved && isResolved === true) continue;
    const group: ThreadGroup = { rootComment: root, replies, isOutdated, isResolved };
    const reviewId = root.pull_request_review_id;
    if (typeof reviewId === "number" && reviewIds.has(reviewId)) {
      if (!threadsByReview.has(reviewId)) threadsByReview.set(reviewId, []);
  ...
  const rank = (c: Card) => (c.kind === "review_event" ? 0 : 1);
  cards.sort((a, b) => {
...
function categorizeChecks(checks: UnifiedCheck[]) {
 ...
 */
function transformProse(s: string, transform: (prose: string) => string): string {
  const nonce = randomBytes(6).toString("hex");
  return s
    .split(/(```[\s\S]*?```)/g)
    .map((part, i) => {
      if (i % 2 === 1) return part; // fenced block — byte-faithful
      const spans: string[] = [];
 ...
 */
function maskedTransform(s: string, transform: (masked: string) => string): string {
  const nonce = randomBytes(6).toString("hex");
  const stash: string[] = [];
  const masked = s
    .replace(/```[\s\S]*?```/g, (m) => {
 ...
 */
function filterProseLines(s: string, keep: (line: string) => boolean): string {
  return s
    .split(/(```[\s\S]*?```)/g)
    .map((part, i) =>
      i % 2 === 1 ? part : part.split("\n").filter(keep).join("\n")
    )
    .join("");
}
...
const DETAILS_FURNITURE: RegExp[] = [
 ...
 */
function processDetailsBlocks(s: string, findingHeader?: string): string {
  // De-quote blockquoted <details> regions (line-wise, one `>` level).
  {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inBlock && /^[ \t]*>/.test(lines[i])) lines[i] = un;
...
/**
 * Number the rows of ```diff fences that arrive UNNUMBERED inside a comment
 * body (qodo findings) — anchored by the location the bot printed right above
 * the fence (`path[R496-504]`, R = right/new-file side) and VERIFIED against
 * the local repository when available: the fence's right-side rows are aligned
 * to the actual head blob (`git show headSha:path`), so the gutter shows real
 * file lines, not just the bot's claim. Deletion rows resolve their old-file
 * numbers from the real `git diff baseSha..headSha` for that file.
 *
 * Conservative on purpose: a fence is only touched when (a) it is a ```diff
 * fence, (b) no row is already numbered, and (c) a `path[R…]`-shaped inline
 * code annotation exists in the prose before it. When neither the local blob
 * nor the diff can resolve a row, the annotation start is trusted for the
 * right side and deletion gutters stay blank — never invented.
 */
interface FenceNumberCtx {
  headLines(path: string): string[] | undefined;
  diffRows(path: string): NumberedRow[] | undefined;
}

/** Lazy, cached local-repo reader for numberAnnotatedDiffFences. */
function makeFenceNumberCtx(gitRoot: string, baseSha: string, headSha: string): FenceNumberCtx {
  const headCache = new Map<string, string[] | undefined>();
  const diffCache = new Map<string, NumberedRow[] | undefined>();
  return {
    headLines(path: string) {
      if (!headCache.has(path)) headCache.set(path, readBlobLines(gitRoot, `${headSha}:${path}`));
      return headCache.get(path);
    },
    diffRows(path: string) {
      if (!diffCache.has(path)) {
        let rows: NumberedRow[] | undefined;
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = i + 1; j < lines.length && !lines[j].startsWith("@@"); j++) {
              const l = lines[j];
              if (l.startsWith("-")) rows.push({ oldNum: oldNum++, newNum: null, marker: "-", content: l.slice(1) });
              else if (l.startsWith("+")) rows.push({ oldNum: null, newNum: newNum++, marker: "+", content: l.slice(1) });
              else if (l.startsWith(" ")) rows.push({ oldNum: oldNum++, newNum: newNum++, marker: " ", content: l.slice(1) });
              else if (l.startsWith("\\")) continue; // "\ No newline at end of file"
              else break; // next file header
            }
          }
...
function numberAnnotatedDiffFences(
  s: string, ctx?: FenceNumberCtx,
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    if (bodyLines.some((l) => /^\s*\d+ [+\- ]/.test(l))) continue; // already numbered
    ...
    if (qodoAnns.length > 0) {
      ...
      if (crAnns.length > 0) {
        ...
        if (heads.length > 0) path = heads[heads.length - 1];
      }
    }
    ...
    if (path == null || annStart == null) continue;

    const rows: NumberedRow[] = bodyLines.map((l) => {
      if (l.startsWith("-")) return { oldNum: null, newNum: null, marker: "-" as const, content: l.slice(1) };
      if (l.startsWith("+")) return { oldNum: null, newNum: null, marker: "+" as const, content: l.slice(1) };
      const content = l.startsWith(" ") ? l.slice(1) : l;
      return { oldNum: null, newNum: null, marker: " " as const, content };
    });
    ...
    const alignAt = (contents: string[]): number | null => {
      if (!hl || contents.length === 0) return null;
      const matchesAt = (w: number): boolean => {
        if (w < 1 || w - 1 + contents.length > hl.length) return false;
        for (let k = 0; k < contents.length; k++) {
          if (hl[w - 1 + k] !== contents[k]) return false;
        }
        return true;
      };
      for (let d = 0; d <= 300; d++) {
        for (const w of d === 0 ? [annStart] : [annStart - d, annStart + d]) {
          if (matchesAt(w)) return w;
        }
      }
      return null;
    };
    ...
    if (leftStart != null) {
      ...
      for (const r of rows) {
        if (r.marker === "-") { r.oldNum = oldNum++; }
        else if (r.marker === "+") { r.newNum = newNum++; }
        else { r.oldNum = oldNum++; r.newNum = newNum; newNum++; }
      }
    } else {
      ...
      for (const r of rows) {
        if (r.marker === "-") {
          ...
          if (drows) {
            let best: NumberedRow | null = null;
            for (const d of drows) {
              if (d.marker !== "-" || d.content !== r.content || d.oldNum == null) continue;
              if (best === null || Math.abs(d.oldNum - newNum) < Math.abs(best.oldNum! - newNum)) best = d;
            }
            if (best) r.oldNum = best.oldNum;
          }
        } else {
          r.newNum = newNum;
          if (r.marker === " ") r.oldNum = newNum;
          newNum++;
        }
      }
    }
 ...
 */
function stripLinkDebris(body: string): string {
  ...
  body.split(/(```[\s\S]*?```)/g).forEach((part, i) => {
    if (i % 2 === 1) return; // fence — definitions inside code don't count
    for (const m of part.matchAll(LINK_DEF_LINE)) defined.add(m[1].trim().toLowerCase());
  });
  ...
  return transformProse(body, (seg) => {
    ...
    s = s.replace(/[ \t]*!\[([^\]]*)\]\[([^\]]*)\]/g, (full: string, alt: string, ref: string) => {
    ...
    s = s.replace(/\[([^\]]+)\]\[([^\]]*)\]/g, (full: string, text: string, ref: string) => {
...
export function cleanCommentBody(raw: string, findingHeader?: string): string {
  ...
  s = transformProse(s, (seg) =>
    seg.replace(/<pre>\s*([\s\S]*?)\s*<\/pre>/gi,
  ...
  s = transformProse(s, (seg) =>
    seg
      ...
      .replace(/<(?:b|strong)>\s*<(?:i|em)>([\s\S]*?)<\/(?:i|em)>\s*<\/(?:b|strong)>/gi,
      ...
      // the URL.
      .replace(/<code>([\s\S]*?)<\/code>/gi, (_m: string, x: string) => {
        const text = x
          .replace(/\[((?:[^\[\]]|\[[^\]]*\])*)\]\((?:[^()]|\([^()]*\))*\)/g, "$1")
          .replace(/`/g, "").trim();
        return text ? "`" + text + "`" : "";
      })
      .replace(/<h[1-6]>([\s\S]*?)<\/h[1-6]>/gi, (_m: string, x: string) => `\n**${x.trim()}**\n`)
      .replace(/<li>([\s\S]*?)<\/li>/gi, (_m: string, x: string) => `- ${x.trim()}\n`)
      .replace(/<br\s*\/?>/gi, "\n")
  );
  ...
  s = transformProse(s, (seg) =>
    seg.replace(/<\/?(?:details|summary|div|span|sub|sup|p|pre|kbd|samp|blockquote|ul|ol|li|h[1-6]|code|strong|b|em|i|hr|abbr|small|table|thead|tbody|tr|td|th)\b[^>]*>/gi, "")
  );
  ...
  const dropLine = (line: string): boolean => {
    ...
    if (/\[[^\]]*\]\([^)]*(?:twitter\.com\/intent|x\.com\/intent|mastodon\.[^/]+\/share|reddit\.com\/submit|linkedin\.com\/sharing)/i.test(t)) return true;
    if (/^\[!(?:note|tip|important|warning|caution)\]$/i.test(t)) return true;
    if (/^copilot uses ai\.?\s*check for mistakes\.?$/i.test(t)) return true;
    if (/^view reviewed changes\s*>?$/i.test(t)) return true;
    if (/thanks for using \[?coderabbit/i.test(t)) return true;
    if (/comment\s+`?@coderabbitai/i.test(t)) return true;
    if (/more reviews will be available in/i.test(t)) return true;
    if (/run out of usage credits/i.test(t)) return true;
    if (/review limit reached/i.test(t)) return true;
    if (/couldn'?t start this review/i.test(t)) return true;
    if (/reached your pr review rate limit/i.test(t)) return true;
    if (/space out your commits/i.test(t)) return true;
    if (/it'?s free for oss/i.test(t)) return true;
    if (/consumer version of .*gemini code assist/i.test(t)) return true;
    if (/code review activity will officially cease/i.test(t)) return true;
    if (/for more details on the timeline and next steps/i.test(t)) return true;
    ...
    if (/^(?:\[PR-#\d+\]\([^)]*\)|PR-#\d+)$/.test(t)) return true;
    if (/customize macroscope'?s approvability policy/i.test(t)) return true;
    ...
    // review content.
    if (/^\**\s*code review by qodo\s*\**$/i.test(t)) return true;
    if (/^(?:`[^`]*\(\d+\)`\s*){2,}$/.test(t)) return true;
    return false;
  };
  ...
  s = transformProse(s, (seg) =>
    seg.replace(/^>\s*$/gm, "").replace(/[ \t]+$/gm, "").replace(/\n{3,}/g, "\n\n")
  ).trim();
...
export async function renderPR(data: PRData, options: PRRendererOptions): Promise<void> {
  ...
  let target = out;
  const w = (line: string = "") => target.push(line);
  ...
  const submittedReviewers = [...new Set(reviews.map((r: any) => r.user.login))];
  const requestedNames = (pr.requested_reviewers || []).map((r: any) => r.login);
  const allReviewerNames = [...new Set([...requestedNames, ...submittedReviewers])];
  const reviewers = allReviewerNames.length > 0
    ? allReviewerNames.map((name: string) => {
        const reviewer = [...(pr.requested_reviewers || []), ...reviews.map((r: any) => r.user)]
          .find((u: any) => u.login === name);
        if (/copilot/i.test(name)) return name;
  ...
  const assignees = pr.assignees?.length > 0 ? pr.assignees.map((a: any) => a.login) : ["No one"];
  const labels = pr.labels?.length > 0 ? pr.labels.map((l: any) => l.name) : ["None"];
  const milestone = pr.milestone ? pr.milestone.title : "None";

  const lineAnchor = (root: any): string => {
    const line = root.line ?? root.original_line;
    const startLine = root.start_line ?? root.original_start_line;
    if (startLine && line && startLine !== line) return `lines +${startLine} to +${line}`;
    if (line) return `line +${line}`;
    return "line (position unknown)";
  };

  const conclusionLabel = (c: string | null): string => {
    switch (c) {
  ...
  const reviewVerb = (rawState: string): string => {
  ...
  const stripSeverityLine = (body: string): string =>
    filterProseLines(body, (line) =>
      !/^\s*\**\s*severity\s*:?\s*\**\s*(?:low|medium|high|critical|info|minor|major)\s*\**\s*$/i.test(line)
    );
  ...
  const writeThreadMd = (thread: ThreadGroup, resolved: boolean, insideReview = false): void => {
  ...
  const conversationCount =
    ...
    reviews.filter((r: any) => r.body && r.body.trim()).length;
  ...
  // divider stub.
  const commentBuf: string[] = [];
  const resolvedThreads: ThreadGroup[] = [];
  let n = 1;
  target = commentBuf;
  for (const card of cards) {
    ...
    let reviewThreads: ThreadGroup[] = [];
    if (card.kind === "review_event") {
      for (const t of card.threads || []) {
        if (t.isResolved === true) resolvedThreads.push(t);
        else reviewThreads.push(t);
      }
    }
  ...
  if (failed.length + inProgress.length + passed.length > 0) blank();
  ...
  if (pr.merged) {
    ...
    if (pr.mergeable_state === "dirty") reasons.push("merge conflicts");
    if (changesRequested.length > 0) reasons.push(`${changesRequested.length} change${changesRequested.length !== 1 ? "s" : ""} requested`);
    if (pendingReviews.length > 0) reasons.push(`${pendingReviews.length} pending review${pendingReviews.length !== 1 ? "s" : ""}`);
    if (inProgress.length > 0) reasons.push(`${inProgress.length} check${inProgress.length !== 1 ? "s" : ""} in progress`);
    if (failed.length > 0) reasons.push(`${failed.length} check${failed.length !== 1 ? "s" : ""} failed`);
...
function extractFailureLines(text: string, maxLines: number = 20): string[] {
...
async function fetchJobLog(
  owner: string, repo: string, jobId: string, token: string,
  rl: RateLimitState = createRateLimitState()
): Promise<string | null> {
  try {
    await waitForRateLimit(rl);
 ...
 */
function cleanActionsLog(text: string): string {
...
export async function renderReport(data: PRData, options: PRRendererOptions): Promise<void> {
  ...
  const out: string[] = [];
  const w = (line: string = "") => out.push(line);
  ...
  if (failed.length > 0) {
    ...
    for (const fc of failed) {
      ...
      if (fc.checkRunId) {
        try {
          const allAnns = await fetchAllPages(
            `${base}/check-runs/${fc.checkRunId}/annotations`, token, rl
          );
          ...
          const failureAnns = allAnns.filter(
            (a: any) => (a.annotation_level || "failure").toLowerCase() === "failure"
          );
          ...
          const isGenericStepFailure = (a: any) => {
          ...
          const anns = failureAnns.filter((a: any) => !isGenericStepFailure(a));
                ...
                if (ann.path && ann.start_line) {
                  try {
                    const fileData = await apiFetch(
                      `${base}/contents/${ann.path}?ref=${headSha}`, token, rl
                    );
      ...
      if (!hasDetails && fc.checkRunId) {
        try {
          const checkDetail = await apiFetch(
            `${base}/check-runs/${fc.checkRunId}`, token, rl
          );
      ...
      if (!hasDetails) {
        if (genericAnns.length > 0) {
          ...
          for (const ann of genericAnns) {
            ...
            if (loc) w(`      [${ann.annotation_level || "failure"}] ${loc}`);
            w(`      ${ann.message || ann.title || "(no message)"}`);
          }
 ...
 */
function classifyConflict(region: ConflictRegion): string {
  ...
  if (region.ancestorContent !== undefined) {
    ...
    if (baseChanged && incomingChanged) {
      if (ancestorEmpty) return "Both sides added content to this region";
      if (baseEmpty) return "Baseline deleted this region; incoming modified it";
      if (incomingEmpty) return "Incoming deleted this region; baseline modified it";
      return "Both sides modified this region";
    }
    if (baseChanged) return "Only baseline modified this region (incoming matches ancestor)";
    if (incomingChanged) return "Only incoming modified this region (baseline matches ancestor)";
    return "Neither side modified this region (possible whitespace or merge artifact)";
  }

  // No ancestor — best-effort classification
  if (baseEmpty && !incomingEmpty) return "Baseline has no content; incoming added lines";
  if (!baseEmpty && incomingEmpty) return "Incoming has no content; baseline added lines";
  if (baseEmpty && incomingEmpty) return "Both sides are empty (possible deletion conflict)";
  return "Both sides modified this region";
}
...
function parseConflictMarkers(content: string): ConflictRegion[] {
  ...
  // Pass 1: extract all conflict regions (without context)
  interface RawRegion {
...
function gitExec(args: string[], cwd: string, timeout: number = GIT_LOCAL_TIMEOUT): string {
  try {
...
function gitExecSafe(
  args: string[], cwd: string, timeout: number = GIT_LOCAL_TIMEOUT
...
function cleanupWorktree(gitRoot: string, worktreeDir: string): void {
  try {
 ...
 */
export function gitCommonDir(gitRoot: string): string {
  return execFileSync(
 ...
 */
function fetchPrRefs(
  gitRoot: string, base: string, head: string, spec: PrRefSpec
): FetchedPrRefs {
  ...
  const baseFetch = gitExecSafe(
    ["fetch", "--no-tags", "--force", "origin",
     `+refs/heads/${base}:${baseRef}`],
    gitRoot, GIT_FETCH_TIMEOUT,
  );
  if (!baseFetch.ok) {
    throw new Error(
      `Failed to fetch base ref refs/heads/${base} from origin: ${baseFetch.stderr}`
    );
  }

  // Head — try the PR ref first (works for forks); fall back to the branch.
  let headFetch = gitExecSafe(
    ["fetch", "--no-tags", "--force", "origin",
     `+refs/pull/${spec.pullNumber}/head:${headRef}`],
    gitRoot, GIT_FETCH_TIMEOUT,
  );
  if (!headFetch.ok) {
    headFetch = gitExecSafe(
      ["fetch", "--no-tags", "--force", "origin",
       `+refs/heads/${head}:${headRef}`],
      gitRoot, GIT_FETCH_TIMEOUT,
    );
  }
  if (!headFetch.ok) {
    throw new Error(
      `Failed to fetch PR head — tried refs/pull/${spec.pullNumber}/head and ` +
  ...
  if (spec.baseSha && baseResolved !== spec.baseSha) {
    throw new Error(
      `Base SHA mismatch: PR metadata says ${spec.baseSha} but ` +
  ...
  if (spec.headSha && headResolved !== spec.headSha) {
    throw new Error(
      `Head SHA mismatch: PR metadata says ${spec.headSha} but ` +
 ...
 */
function allocWorktreeDir(gitRoot: string, label: string): string {
...
interface UnmergedEntry {
  stage: number; // 1=ancestor, 2=ours, 3=theirs
  mode: string;
  oid: string;
}

/** Read the unmerged stages for a path. Empty array = no conflict on that path. */
function getUnmergedStages(workTree: string, path: string): UnmergedEntry[] {
...
function getStagedFingerprint(workTree: string, path: string): IndexFingerprint {
  const r = gitExecSafe(["ls-files", "--stage", "--", path], workTree);
  if (!r.ok || !r.stdout) return { kind: "deleted" };
  // First line only — stage-0 entries are unique
  const m = r.stdout.split("\n")[0].match(/^(\d+)\s+([0-9a-f]+)\s+0\t/);
  if (!m) return { kind: "deleted" };
 ...
 */
export function parseCombinedDiffSideMap(
  diffText: string,
): Map<number, { ours: number | null; theirs: number | null }> {
  const map = new Map<number, { ours: number | null; theirs: number | null }>();
  ...
  for (const line of diffText.split("\n")) {
    ...
    if (line.length < 2) { inHunk = false; continue; }  // end of hunk body
    const c1 = line[0], c2 = line[1];
    if ("+- ".indexOf(c1) === -1 || "+- ".indexOf(c2) === -1) { inHunk = false; continue; }
...
export function splitGitLines(text: string): string[] {
  const lines = text.split("\n");
...
export function readBlobLines(cwd: string, spec: string): string[] | undefined {
  try {
...
function scanConflictMarkerLines(
  lines: string[],
): { start: number; ancStart: number; sep: number; end: number }[] {
  const out: { start: number; ancStart: number; sep: number; end: number }[] = [];
  let i = 0;
  while (i < lines.length) {
    if (!lines[i].startsWith("<<<<<<<")) { i++; continue; }
    const start = i;
    ...
    while (j < lines.length) {
      ...
      else if (l.startsWith("=======") && sep === -1) sep = j;
      else if (l.startsWith(">>>>>>>")) { end = j; break; }
      j++;
    }
    if (sep === -1 || end === -1) { i = start + 1; continue; }
...
function locateBlock(
  ...
  if (!blob || block.length === 0) return null;
  const matches: number[] = [];
  for (let i = 0; i + block.length <= blob.length; i++) {
    let ok = true;
    for (let j = 0; j < block.length; j++) {
      if (blob[i + j] !== block[j]) { ok = false; break; }
    }
  ...
  for (let x = 1; x < pool.length; x++) {
    ...
    if (sc > bestScore) { best = pool[x]; bestScore = sc; tie = false; }
 ...
 */
export function buildSideLineMap(
  mergedContent: string,
  combined: Map<number, { ours: number | null; theirs: number | null }> | undefined,
  blobs: { ours?: string[]; base?: string[]; theirs?: string[] },
): SideLineMap {
  const CTX = 3; // shared-context window used only for disambiguation
  const lines = mergedContent.split("\n");
  const regions = scanConflictMarkerLines(lines);
  const map: SideLineMap = new Map();
  ...
  if (combined) {
    for (const [ln, v] of combined) map.set(ln, { ours: v.ours, theirs: v.theirs, base: null });
  }
  ...
  for (let r = 0; r < regions.length; r++) {
    ...
    // OURS block: 1-based result lines begin at (rg.start + 2).
    {
      ...
      if (s != null) minStart.ours = s + oursBlock.length;
    ...
    if (rg.ancStart >= 0) {
      ...
      if (s != null) minStart.base = s + baseBlock.length;
    ...
    // THEIRS block: 1-based result lines begin at (rg.sep + 2).
    {
      ...
      if (s != null) minStart.theirs = s + theirsBlock.length;
 ...
 */
function trialMergeSideMap(
  gitRoot: string, trialCommit: string, filePath: string,
): Map<number, { ours: number | null; theirs: number | null }> | undefined {
  try {
    const cc = execFileSync(
      "git",
      ["diff-tree", "--cc", "-r", `-U${SIDEMAP_CONTEXT}`, trialCommit, "--", filePath],
      { cwd: gitRoot, encoding: "utf-8", timeout: GIT_LOCAL_TIMEOUT,
        stdio: ["pipe", "pipe", "pipe"], maxBuffer: 64 * 1024 * 1024 },
    ).toString();
...
export function extractConflicts(
  gitRoot: string, base: string, head: string,
  prSpec: PrRefSpec = { pullNumber: 0 },
): ConflictFile[] {
  const refs = fetchPrRefs(gitRoot, base, head, prSpec);

  // In-memory trial merge. exit 0 = clean, 1 = conflicts, >1 = merge error.
  let status: number, stdout: string, stderr: string;
  try {
    stdout = execFileSync(
      "git",
      ["merge-tree", "--write-tree", "--name-only", refs.baseRef, refs.headRef],
      {
        cwd: gitRoot, encoding: "utf-8", timeout: GIT_LOCAL_TIMEOUT,
        stdio: ["pipe", "pipe", "pipe"], maxBuffer: 64 * 1024 * 1024,
      },
  ...
  if (status === 0) return [];                 // clean merge
  ...
  let trialCommit: string | null = null;
  try {
    trialCommit = execFileSync(
      "git",
      ["commit-tree", tree, "-p", refs.baseRef, "-p", refs.headRef, "-m", "git-print trial merge"],
      { cwd: gitRoot, encoding: "utf-8", timeout: GIT_LOCAL_TIMEOUT,
        stdio: ["pipe", "pipe", "pipe"], maxBuffer: 16 * 1024 * 1024 },
    ).toString().trim();
    if (!/^[0-9a-f]{7,64}$/.test(trialCommit)) trialCommit = null;
  } catch {
    trialCommit = null;
  }
...
export async function renderConflicts(
  data: PRData,
  options: PRRendererOptions & { gitRoot: string }
): Promise<string | null> {
  const { pr } = data;
...
function buildCommitMessage(prNumber: number, resolved: { file: string; side: string }[]): string {
  const maxFileLen = Math.max(...resolved.map(r => r.file.length));
  const lines = resolved.map(r =>
    `  ${r.file.padEnd(maxFileLen)}  → used ${r.side}`
  );
 ...
 */
function applyPlanInWorkTree(
  workTree: string, plan: ResolvedFile[],
): string | null {
  for (const entry of plan) {
    ...
    // does not.
    const co = gitExecSafe(
      ["checkout-index", "-f", `--stage=${wantedStage}`, "--", entry.path],
      workTree,
    );
    if (!co.ok) {
...
export function validateInWorktree(
  gitRoot: string,
  base: string,
  head: string,
  resolutions: Map<string, "baseline" | "incoming">,
  prSpec: Pick<ResolveOptions, "baseSha" | "headSha" | "pullNumber"> = { pullNumber: 0 },
): ValidationResult {
  ...
  try {
    ...
    const mergeResult = gitExecSafe(
      ["merge", "--no-commit", "--no-ff", refs.baseSha], worktreeDir,
    );
    if (mergeResult.ok) {
      return {
    ...
    const conflictOutput = gitExecSafe(
    ...
    const resolutionPlan: ResolvedFile[] = [];
...
export function applyResolutions(
  ...
  if (userHeadSha !== refs.headSha) {
    ...
    return {
      status: "aborted",
      error:
        `✗ Current branch (${branchName} at ${userHeadSha.slice(0, 12)}) is not at the PR head ` +
  ...
  if (!mergeAlreadyInProgress(gitRoot)) {
    const mergeResult = gitExecSafe(
      ["merge", "--no-commit", "--no-ff", refs.baseSha], gitRoot,
    );
    if (mergeResult.ok) {
  ...
  const replayPlan: ResolvedFile[] = validation.resolutionPlan.map(r => ({
  ...
  for (let i = 0; i < replayPlan.length; i++) {
    ...
    if (expected.kind !== actual.kind ||
      ...
      return {
        status: "aborted",
        error:
          `✗ Resolution fingerprint mismatch for ${replayPlan[i].path}. ` +
...
function mergeAlreadyInProgress(workTree: string): boolean {
...
export function resolveConflicts(opts: ResolveOptions): ResolveResult {
  ...
  const validation = validateInWorktree(
    opts.gitRoot, opts.base, opts.head, opts.resolutions,
    { baseSha: opts.baseSha, headSha: opts.headSha, pullNumber: opts.pullNumber },
  );
  ...
  if (validation.skipped.length > 0) {
    process.stderr.write(
      `  (${validation.skipped.length} file${validation.skipped.length !== 1 ? "s" : ""} skipped — no flag specified)\n`
    );
  }
