...
export async function renderReport(data: PRData, options: PRRendererOptions): Promise<void> {
  ...
  const wFence = (lines: string[], lang = "") => {
    let ticks = 3;
    for (const ln of lines) {
      for (const run of ln.match(/`+/g) ?? []) ticks = Math.max(ticks, run.length + 1);
    }
    const fence = "`".repeat(ticks);
    w(fence + lang);
    for (const ln of lines) w(ln);
    w(fence);
  };
...
export function validateInWorktree(
  ...
  head: string,
  resolutions: Map<string, "baseline" | "incoming">,
  prSpec: Pick<ResolveOptions, "pullNumber"> = { pullNumber: 0 },
): ValidationResult {
